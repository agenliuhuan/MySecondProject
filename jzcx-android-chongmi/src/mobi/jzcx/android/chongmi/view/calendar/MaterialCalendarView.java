package mobi.jzcx.android.chongmi.view.calendar;
...
import mobi.jzcx.android.chongmi.view.calendar.format.ArrayWeekDayFormatter;
import mobi.jzcx.android.chongmi.view.calendar.format.DateFormatTitleFormatter;
import mobi.jzcx.android.chongmi.view.calendar.format.DayFormatter;
import mobi.jzcx.android.chongmi.view.calendar.format.MonthArrayTitleFormatter;
import mobi.jzcx.android.chongmi.view.calendar.format.TitleFormatter;
import mobi.jzcx.android.chongmi.view.calendar.format.WeekDayFormatter;
 ...
 * by taping on it and can page the calendar to a desired date.
 * </p>
 * <p>
 * By default, the range of dates shown is from 200 years in the past to 200
 * years in the future. This can be extended or shortened by configuring the
 * minimum and maximum dates.
 * </p>
 * <p>
 * When selecting a date out of range, or when the range changes so the
 * selection becomes outside, The date closest to the previous selection will
 * become selected. This will also trigger the
 * {@linkplain OnDateSelectedListener}
 * </p>
 * <p>
 * <strong>Note:</strong> if this view's size isn't divisible by 7, the contents
 * will be centered inside such that the days in the calendar are equally
 * square. For example, 600px isn't divisible by 7, so a tile size of 85 is
 * choosen, making the calendar 595px wide. The extra 5px are distributed left
 * and right to get to 600px.
 * </p>
 */
@SuppressLint("NewApi")
public class MaterialCalendarView extends ViewGroup {
	...
	/**
	 * Selection mode that disallows all selection. When changing to this mode,
	 * current selection will be cleared.
	 */
	public static final int SELECTION_MODE_NONE = 0;

	/**
	 * Selection mode that allows one selected date at one time. This is the
	 * default mode. When switching from {@linkplain #SELECTION_MODE_MULTIPLE},
	 * this will select the same date as from {@linkplain #getSelectedDate()},
	 * which should be the last selected date
	 */
	...
	@Retention(RetentionPolicy.RUNTIME)
	@IntDef(flag = true, value = {SHOW_NONE, SHOW_ALL, SHOW_DEFAULTS, SHOW_OUT_OF_RANGE, SHOW_OTHER_MONTHS,
	...
	/**
	 * Show dates from the proceeding and successive months, in a disabled
	 * state. This flag also enables the {@link #SHOW_OUT_OF_RANGE} flag to
	 * prevent odd blank areas.
	 */
	public static final int SHOW_OTHER_MONTHS = 1;

	/**
	 * Show dates that are outside of the min-max range. This will only show
	 * days from the current month unless {@link #SHOW_OTHER_MONTHS} is enabled.
	 */
	public static final int SHOW_OUT_OF_RANGE = 2;

	/**
	 * Show days that are individually disabled with decorators. This will only
	 * show dates in the current month and inside the minimum and maximum date
	 * range.
	 */
	public static final int SHOW_DECORATED_DISABLED = 4;

	/**
	 * The default flags for showing non-enabled dates. Currently only shows
	 * {@link #SHOW_DECORATED_DISABLED}
	 */
	...
	/**
	 * Default tile size in DIPs. This is used in cases where there is no tile
	 * size specificed and the view is set to
	 * {@linkplain ViewGroup.LayoutParams#WRAP_CONTENT WRAP_CONTENT}
	 */
	...
	public MaterialCalendarView(Context context, AttributeSet attrs) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			// If we're on good Android versions, turn off clipping for cool
			// effects
			setClipToPadding(false);
			setClipChildren(false);
		} else {
			// Old Android does not like _not_ clipping view pagers, we need to
			// clip
		...
		try {
			...
			int firstDayOfWeek = a.getInteger(R.styleable.MaterialCalendarView_mcv_firstDayOfWeek, -1);
			if (firstDayOfWeek < 0) {
				// Allowing use of Calendar.getInstance() here as a performance
				// optimization
	...
	private void setupChildren() {
		...
		topbar.addView(title, new LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT,
				MonthView.DEFAULT_DAYS_IN_WEEK - 2));
	 ...
	 * {@linkplain #SELECTION_MODE_SINGLE}
	 * 
	 * @param mode
	 *            the selection mode to change to. This must be one of
	 *            {@linkplain #SELECTION_MODE_NONE},
	 *            {@linkplain #SELECTION_MODE_SINGLE}, or
	 *            {@linkplain #SELECTION_MODE_MULTIPLE}. Unknown values will act
	 *            as {@linkplain #SELECTION_MODE_SINGLE}
	 * @see #getSelectionMode()
		...
		switch (mode) {
			...
			case SELECTION_MODE_SINGLE : {
				...
				if (oldMode == SELECTION_MODE_MULTIPLE) {
					// We should only have one selection now, so we should pick
					// one
	...
	/**
	 * Get the current selection mode. The default mode is
	 * {@linkplain #SELECTION_MODE_SINGLE}
	 * 
	...
	/**
	 * Set the size of each tile that makes up the calendar. Each day is 1 tile,
	 * so the widget is 7 tiles wide and 7 or 8 tiles tall depending on the
	 * visibility of the {@link #topbar}.
	 * 
	 * @param size
	 *            the new size for each tile in pixels
	 */
	...
	/**
	 * @param tileSizeDp
	 *            the new size for each tile in dips
	 * @see #setTileSize(int)
	 */
	...
	/**
	 * @param color
	 *            the new color for the paging arrows
	 */
	...
	/**
	 * @param icon
	 *            the new icon to use for the right paging arrow
	 */
	...
	/**
	 * @return the selected day, or null if no selection. If in multiple
	 *         selection mode, this will return the last selected date
	 */
	...
	/**
	 * @param calendar
	 *            a Calendar set to a day to select. Null to clear selection
	 */
	...
	/**
	 * @param date
	 *            a Date to set as selected. Null to clear selection
	 */
	...
	/**
	 * @param calendar
	 *            a Calendar to change. Passing null does nothing
	 * @param selected
	 *            true if day should be selected, false to deselect
	 */
	...
	/**
	 * @param date
	 *            a Date to change. Passing null does nothing
	 * @param selected
	 *            true if day should be selected, false to deselect
	 */
	...
	/**
	 * @param day
	 *            a CalendarDay to change. Passing null does nothing
	 * @param selected
	 *            true if day should be selected, false to deselect
	 */
	...
	/**
	 * @param calendar
	 *            a Calendar set to a day to focus the calendar on. Null will do
	 *            nothing
	 */
	...
	/**
	 * @param day
	 *            a CalendarDay to focus the calendar on. Null will do nothing
	 */
	...
	/**
	 * @param calendar
	 *            set the maximum selectable date, null for no maximum
	 */
	...
	/**
	 * The default value is {@link #SHOW_DEFAULTS}, which currently is just
	 * {@link #SHOW_DECORATED_DISABLED}. This means that the default visible
	 * days are of the current month, in the min-max range.
	 * 
	 * @param showOtherDates
	 *            flags for showing non-enabled dates
	 * @see #SHOW_ALL
	 * @see #SHOW_NONE
	 * @see #SHOW_DEFAULTS
	 ...
	 * 
	 * @param formatter
	 *            the new formatter, null for default
	 */
	...
	/**
	 * Set a
	 * {@linkplain com.prolificinteractive.materialcalendarview.format.WeekDayFormatter}
	 * with the provided week day labels
	 * 
	 * @param weekDayLabels
	 *            Labels to use for the days of the week
	 ...
	 *            new formatter to use, null to use default formatter
	 */
	...
	/**
	 * Set a
	 * {@linkplain com.prolificinteractive.materialcalendarview.format.TitleFormatter}
	...
	/**
	 * Sets the visibility {@link #topbar}, which contains the previous month
	 * button {@link #buttonPast}, next month button {@link #buttonFuture}, and
	 * the month title {@link #title}.
	 * 
	 * @param visible
	 *            Boolean indicating if the topbar is visible
	 */
	 ...
	 * 
	 * @param day
	 *            The first day of the week as a java.util.Calendar day
	 *            constant.
	...
	/**
	 * @return The first day of the week as a {@linkplain Calendar} day
	 *         constant.
	 */
	...
	/**
	 * By default, the calendar will take up all the space needed to show any
	 * month (6 rows). By enabling dynamic height, the view will change height
	 * dependant on the visible month.
	 * 
	 * This means months that only need 5 or 4 rows to show the entire month
	 * will only take up that many rows, and will grow and shrink as necessary.
	 * 
	 * @param useDynamicHeight
	 *            true to have the view different heights based on the visible
	 *            month
	 */
	...
	/**
	 * Remove a specific decorator instance. Same rules as
	 * {@linkplain List#remove(Object)}
	 * 
	 * @param decorator
	 *            decorator to remove
	 */
	...
	/**
	 * Invalidate decorators after one has changed internally. That is, if a
	 * decorator mutates, you should call this method to update the widget.
	 */
	 ...
	 * @param selected
	 *            true if the day is now currently selected, false otherwise
	 */
	...
	/**
	 * Call by MonthView to indicate that a day was clicked and we should handle
	 * it
	 * 
	 * @param date
	 *            date of the day that was clicked
	 * @param nowSelected
	...
	/**
	 * Called by the adapter for cases when changes in state result in dates
	 * being unselected
	 * 
	 * @param date
	 *            date that should be de-selected
	 */
	...
	protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
		...
		// We need to disregard padding for a while. This will be added back
		// later
		...
		/*
		 * The default height of the calendar component is
		 * MonthView.DEFAULT_MONTH_TILE_HEIGHT rows, but we need to hide 1 if
		 * the actual date is 5 week long
		 */
		if (mDynamicHeightEnabled && adapter != null && pager != null) {
			...
			weekCount = cal.get(Calendar.WEEK_OF_MONTH) + 1; // + 1 because of
																// the week days
		}
		...
		// We clamp inline because we want to use un-clamped versions on the
		// children
		...
		for (int i = 0; i < count; i++) {
			...
			int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(MonthView.DEFAULT_DAYS_IN_WEEK * measureTileSize,
					MeasureSpec.EXACTLY);

			int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(p.height * measureTileSize, MeasureSpec.EXACTLY);
	 ...
	 *            Size we want to be
	 * @param spec
	 *            Measure spec to clamp against
	 * @return the appropriate size to pass to
	 *         {@linkplain View#setMeasuredDimension(int, int)}
	 */
	...
	/**
	 * Simple layout params for MaterialCalendarView. The only variation for
	 * layout is height.
	 */
	protected static class LayoutParams extends MarginLayoutParams {

		/**
		 * Create a layout that matches parent width, and is X number of tiles
		 * high
		 * 
		 * @param tileHeight
		 *            view height in number of tiles
		 */
	 ...
	 * 
	 * @param pagingEnabled
	 *            pass false to disable paging, true to enable (default)
	 */
	...
	/**
	 * @return true if swiping months is enabled, false if disabled. Default is
	 *         true.
	 */
