...
public class TVPageIndicator extends LinearLayout implements ViewPager.OnPageChangeListener {
	...
	public TVPageIndicator(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public TVPageIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);

		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.PageIndicator, 0, 0);
		try {
			mIndicatorSpacing = a.getDimensionPixelSize(R.styleable.PageIndicator_indicator_spacing,
	...
	private void addIndicator(int count) {
		...
		if (mIndicatorType == IndicatorType.CIRCLE) {
			...
			textView.setBackgroundResource(R.drawable.fraction_bg);
	...
	private void updateIndicator(int position) {
		if (mIndicatorTypeChanged || mActivePosition != position) {
			...
			if (mIndicatorType == IndicatorType.CIRCLE) {
				...
				// textView.setText(String.format("%d/%d", position + 1,
				// (int)textView.getTag()));
				if (paperCount != 0) {
					textView.setText(String.format("%d/%d", position + 1, paperCount));
				} else {
					textView.setText(String.format("%d/%d", position + 1, mViewPager.getAdapter().getCount()));
				}
