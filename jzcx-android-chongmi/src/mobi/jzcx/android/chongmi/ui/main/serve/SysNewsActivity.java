...
public class SysNewsActivity extends BaseExActivity implements ReportClickListener {
					...
					try {
						...
						if (null != extraJson && extraJson.length() > 0) {
							...
							if (msgtype.equals("System")) {
								pageindex = 1;
								sendMessage(YSMSG.REQ_GETSysNOTICE, pageindex, 0, null);
	...
	private void initView() {
		initTitleBar();
		xlistView = (SwipeMenuListView) findViewById(R.id.sysnewsXList);
		adapter = new SystemNewsAdapter(getBaseContext());
		adapter.setReportClick(this);
		emptyview = findViewById(R.id.sysnewsemptyLL);
		...
		xlistView.setOnMenuItemClickListener(new OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(int position, SwipeMenu menu, int index) {
				if (adapter != null) {
					SystemNoticeObject noticeObj = adapter.getItem(position);
					sendMessage(YSMSG.REQ_NOTICEREMOVE, 0, 0, noticeObj.getNoticeMsgId());
					curSystemNoticeObject = noticeObj;
				}
				return false;
			}
		});
		xlistView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (adapter != null) {
					SystemNoticeObject noticeObj = adapter.getItem(position);
					SystemNewsDetailActivity.startActivity(mActivity, noticeObj);
				}
			}
		});
	...
	private int getWidth() {
		int width = CommonUtils.getScreenWidth(mActivity) * 1800 / 10000;
		return width;
	}
	...
	private void initTitleBar() {
		...
		mTitleBar.mTitle.setText(getString(R.string.sysnews_title));
		...
		public void onClick(View v) {
			...
			switch (v.getId()) {
				case R.id.reportOrDeleteBtn :
					sendMessage(YSMSG.REQ_NOTICEREMOVEALL, 0, 0, "2");
					break;
				default :
					break;
			}
...
//								}
							...
							for (int i = 0; i < size; i++) {
								SystemNoticeObject noticeObj = OJMFactory.createOJM().fromJson(array.getString(i),
										SystemNoticeObject.class);
								noticeList.add(noticeObj);
								// if (i == size - 1) {
								// sb.append(noticeObj.get_id());
								// } else {
								// sb.append(noticeObj.get_id() + ",");
								// }
			...
			case YSMSG.RESP_NOTICEREMOVE :
				if (msg.arg1 == 200) {
					if (curSystemNoticeObject != null) {
						YSToast.showToast(mActivity, getString(R.string.msg_delete_success));
						noticeList.remove(curSystemNoticeObject);
						updateList();
					}
				} else {
					YSToast.showToast(mActivity, String.valueOf(msg.obj));
	...
	private static final Comparator<SystemNoticeObject> COMPARATOR = new Comparator<SystemNoticeObject>() {
		@Override
		public int compare(SystemNoticeObject lhs, SystemNoticeObject rhs) {
			Date date1 = CommonUtils.getCreateDate(lhs.getCreateTime());
			Date date2 = CommonUtils.getCreateDate(rhs.getCreateTime());
