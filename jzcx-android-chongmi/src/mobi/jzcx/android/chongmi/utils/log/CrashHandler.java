...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				YSToast.showToast(mContext, mContext.getString(R.string.crash_text));
				Looper.loop();
			}
		}.start();
		// 收集设备参数信息
		...
		// 保存日志文件
		LogUtils.i(TAG, "ex:" + ex.toString() + "--" + ex.getLocalizedMessage());
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		while (cause != null) {
			LogUtils.i(TAG, "cause:" + cause.toString() + "--");
