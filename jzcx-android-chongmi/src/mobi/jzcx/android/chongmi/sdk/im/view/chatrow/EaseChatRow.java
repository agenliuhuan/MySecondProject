...
import mobi.jzcx.android.chongmi.sdk.im.view.EaseChatMessageList.MessageListItemClickListener;
import mobi.jzcx.android.chongmi.sdk.im.view.EaseMessageAdapter;
...
public abstract class EaseChatRow extends LinearLayout {
	...
	private void setUpBaseView() {
		...
		ImContactObject contact = contactDao.getImContactById(message.getFrom());
		UserObject myself = CoreModel.getInstance().getMyself();
		// 设置头像和nick
		if (message.direct == Direct.SEND) {
			ImageLoaderHelper.displayAvatar(myself.getIcoUrl(), userAvatarView);
			// EaseUserUtils.setUserAvatar(context,
			// EMChatManager.getInstance().getCurrentUser(), userAvatarView);
			// 发送方不显示nick
			...
			if (contact != null) {
				if (!CommonTextUtils.isEmpty(contact.getContactName())) {
					usernickView.setText(contact.getContactName());
				}
				if (!CommonTextUtils.isEmpty(contact.getContactAvatar())) {
					ImageLoaderHelper.displayAvatar(contact.getContactAvatar(), userAvatarView);
				}
			}
	...
	private void setClickListener() {
		...
		userAvatarView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (itemClickListener != null) {
					if (message.direct == Direct.SEND) {
						itemClickListener.onUserAvatarClick(IMInitHelper.getInstance().getCurrentUsernName());
					} else {
					...
					if (message.getError() == EMError.MESSAGE_SEND_INVALID_CONTENT) {
						YSToast.showToast(
								...
								activity.getString(R.string.send_fail)
										+ activity.getString(R.string.error_send_invalid_content));
					} else if (message.getError() == EMError.MESSAGE_SEND_NOT_IN_THE_GROUP) {
						YSToast.showToast(
								activity,
								activity.getString(R.string.send_fail)
										+ activity.getString(R.string.error_send_not_in_the_group));
					} else {
						YSToast.showToast(
								activity,
								activity.getString(R.string.send_fail)
