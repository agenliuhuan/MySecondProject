package mobi.jzcx.android.core.view.stickylistheaders;
...
import mobi.jzcx.android.core.view.stickylistheaders.WrapperViewList.LifeCycleListener;
 ...
 */
public class StickyListHeadersListView extends FrameLayout implements Pullable {
	...
	public boolean canPullDown() {
		if (canPullDown) {
			if (getCount() == 0) {
				// 没有item的时候也可以下拉刷新
				return true;
			} else if (mList != null && mList.getFirstVisiblePosition() == 0 && mList.getChildAt(0).getTop() >= 0) {
				// 滑到ListView的顶部了
				return true;
			} else
				return false;
	...
	public boolean canPullUp() {
		if (canPullUp) {
			if (getCount() == 0) {
				...
				// 滑到底部了
				if (mList != null && mList.getChildAt(getLastVisiblePosition() - getFirstVisiblePosition()) != null
						&& mList.getChildAt(getLastVisiblePosition() - getFirstVisiblePosition()).getBottom() <= getMeasuredHeight())
					return true;
			}
