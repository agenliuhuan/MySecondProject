...
public class PullToZoomListViewEx extends PullToZoomBase<ListView> implements AbsListView.OnScrollListener {
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		if (mRootView.getChildCount() > 2 && mRootView.getFirstVisiblePosition() == 0) {
			Log.d(TAG, "onScrollChanged " + mRootView.getChildAt(1).getY());
		}
	}
	...
	public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		if (mZoomView != null && !isHideHeader() && isPullToZoomEnabled()) {
			int f = mHeaderHeight - mHeaderContainer.getBottom();
			Log.d(TAG, "f: " + f);
			...
			if (scrollListener != null) {
				scrollListener.scrollTo(view, firstVisibleItem, visibleItemCount, totalItemCount, f);
			}
		}

	}

	ListViewScrollListener scrollListener;

	public void setScrollListener(ListViewScrollListener scrollListener) {
