...
import mobi.jzcx.android.core.R;
...
public class SwipeLayout extends FrameLayout {
	...
	private Map<View, Rect> mViewBoundCache = new HashMap<View, Rect>();// save
																		// all
																		// children's
																		// bound,
																		// restore
	...
	private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
		...
		public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
			...
			int evLeft = surfaceView.getLeft(), evRight = surfaceView.getRight(), evTop = surfaceView.getTop(), evBottom = surfaceView
					.getBottom();
		...
		if (gravity > 0) {
			...
			for (Map.Entry<DragEdge, View> entry : mDragEdges.entrySet()) {
				if (entry.getValue() == null) {
					// means used the drag_edge attr, the no gravity child
					// should be use set
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			sX = ev.getRawX();
			sY = ev.getRawY();
			// if the swipe is in middle state(scrolling), should intercept the
			// touch
			if (getOpenStatus() == Status.Middle) {
				mIsBeingDragged = true;
			}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (!beforeCheck && mIsBeingDragged) {
				// let children has one chance to catch the touch, and request
				// the swipe not intercept
				// useful when swipeLayout wrap a swipeLayout or other gestural
				// layout
				return false;
			}
			break;
	...
	private void performAdapterViewItemClick() {
		...
		if (t instanceof AdapterView) {
			...
			if (p != AdapterView.INVALID_POSITION) {
				view.performItemClick(view.getChildAt(p - view.getFirstVisiblePosition()), p, view.getAdapter()
						.getItemId(p));
			}
		}
	}
	...
	private boolean performAdapterViewItemLongClick() {
		...
		if (t instanceof AdapterView) {
			...
			try {
				...
				handled = ((Boolean) m.invoke(view, SwipeLayout.this, p, vId)).booleanValue();
	...
	/**
	 * @return all bottomViews: left, top, right, bottom (may null if the edge
	 *         is not set)
	 */
