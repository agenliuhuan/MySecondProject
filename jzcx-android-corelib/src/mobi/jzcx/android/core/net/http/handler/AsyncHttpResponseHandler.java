...
public class AsyncHttpResponseHandler {
	...
	public void sendResponseMessage(HttpResponse response, HttpUriRequest request) {
		...
		if (200 != responseCode) {
			...
			HttpResponseException exception = new HttpResponseException(status.getStatusCode(), status.getStatusCode()
		...
		try {
			...
			printRequest(request.getURI().toString(), "", e1.getMessage());
			e1.printStackTrace();
